...
//  m_measurements.push_back ("lostPackets");
  ...
  for (MeasurementDataset::const_iterator it = m_mMeasurementDataset.begin (); it != m_mMeasurementDataset.end (); ++it)
   {
    ...
    //std::string imageName = ImageName (i);
    Gnuplot gnuplot (measurement + ".svg", measurement);
    gnuplot.SetTerminal ("svg"); // non detected terminal
    gnuplot.SetLegend ("Time [sec] ", "Y");
    ...
    std::string file = "net-measure-" + measurement + ".plt";
 ...
 void
 NetMeasure::ExpandFlowStats (uint32_t newSize)
 {
    ...
    for (MeasurementDataset::const_iterator ite = m_mMeasurementDataset.begin (); ite != m_mMeasurementDataset.end (); ++ite)
     {
      ...
      double data;
      if (measurement.compare ("rxBytes") == 0)
       {
        ...
        if (measurement.compare ("delayHistogram") == 0)
         {
          ...
          // style histograms not implemented in gnuplot

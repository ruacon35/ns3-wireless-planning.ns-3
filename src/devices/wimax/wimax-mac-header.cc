...
void GenericMacHeader::Serialize (Buffer::Iterator start) const
{
  ...
  uint8_t crc = CRC8Calculate (headerBuffer, sizeof(headerBuffer) - 1);
  headerBuffer[5] = crc;
...
uint32_t GenericMacHeader::Deserialize (Buffer::Iterator start)
{
  ...
  c_hcs = CRC8Calculate (headerBuffer, sizeof(headerBuffer) - 1);
...
{
  /*
   * AI:The size of the BandwidthRequest mac header is always 6 bytes
   */
  return 6;
}
...
void BandwidthRequestHeader::Serialize (Buffer::Iterator start) const
{
  ...
  headerBuffer[5] = CRC8Calculate (headerBuffer, sizeof(headerBuffer) - 1);

...
public:
  ...
  // void SetSchedulingType (uint8_t schedulingType);
  ServiceFlow *
  GetServiceFlow (void);
...
private:
  ...
  u_int32_t m_size; /* Number of minislots requested */
  enum ServiceFlow::SchedulingType m_schedulingType; /* Scheduling type of flow */

  u_char m_flag; /* To delete or not..*/
  u_char m_retryCount;
...
/* auxiliar struct to compute the priority of the rtPS and nrtPS in

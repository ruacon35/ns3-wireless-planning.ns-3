...
TypeId WimaxNetDevice::GetTypeId (void)
{
  ...
  tid =
    ...
    .AddTraceSource ("Rx", "Receive trance", MakeTraceSourceAccessor (&WimaxNetDevice::m_traceRx))
...
{
  SendParams * params = new OfdmSendParams (burst, modulationType, m_direction, m_symbolIndex);
  m_phy->Send (params);
  delete params;
}

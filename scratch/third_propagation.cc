...
main (int argc, char *argv[])
{
  ...
  for (uint8_t i = 0; i < apDevices.GetN(); i++) {
    Ptr< WifiNetDevice > wifiNetDev;
    Ptr< NetDevice > netDev;
    netDev = apDevices.Get (i);
    wifiNetDev = netDev->GetObject<WifiNetDevice > ();
    Ptr< WifiMac > mac0;
    mac0 = wifiNetDev->GetMac ();
    Time ackTimeout = Seconds(100e-6);    
    mac0->SetAckTimeout (ackTimeout);
    //Time slotTime = Seconds(100e-3);    
    //mac0->SetSlot (slotTime);
  }
  ...
  positionAlloc2->Add (Vector (6000.0, 0.0, 0.0)); 
  mobility.SetPositionAllocator (positionAlloc2); 
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  //mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel",
//    "Position", VectorValue (Vector(33330.0, 0.0, 0.0)));

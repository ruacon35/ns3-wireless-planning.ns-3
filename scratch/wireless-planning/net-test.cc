...
namespace ns3
{
 ...
 void
 NetTest::ApplicationSetup (std::string client, std::string server, double start, double stop, std::string rate, uint32_t packetSize, AppState* appState)
 {
  ApplicationSetup (client, server, 9, start, stop, rate, packetSize, appState);
 }
                            ...
                            uint32_t packetSize, AppState *appState)
 {
  ...
  oss << "/NodeList/" << clientNode->GetId () <<
          "/ApplicationList/" << clientNode->GetNApplications () - 1 <<
